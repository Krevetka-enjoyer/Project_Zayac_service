...
build-debug build-release: build-%: cmake-%
	@cmake --build build_$* -j $(NPROCS) --target Project_Zayac

# Test
.PHONY: test-debug test-release
test-debug test-release: test-%: build-%
#	@cmake --build build_$* -j $(NPROCS) --target Project_Zayac_unittest
#	@cmake --build build_$* -j $(NPROCS) --target Project_Zayac_benchmark
...
service-start-debug service-start-release: service-start-%: build-%
	@cd ./build_$* && $(MAKE) start-Project_Zayac
...
install-debug install-release: install-%: build-%
	@cd build_$* && \
		cmake --install . -v --component Project_Zayac
...
--in-docker-start-debug --in-docker-start-release: --in-docker-start-%: install-%
	@/root/.local/bin/Project_Zayac \
		--config /root/.local/etc/Project_Zayac/static_config.yaml

# Build and run service in docker environment
.PHONY: docker-start-service-debug docker-start-service-release
docker-start-service-debug docker-start-service-release: docker-start-service-%:
	@docker-compose run -p 8080:8080 --rm Project_Zayac-container $(MAKE) -- --in-docker-start-$*
...
docker-cmake-debug docker-build-debug docker-test-debug docker-clean-debug docker-install-debug docker-cmake-release docker-build-release docker-test-release docker-clean-release docker-install-release: docker-%:
	docker-compose run --rm Project_Zayac-container $(MAKE) $*
